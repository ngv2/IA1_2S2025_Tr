...
export default function PatientIntakeAndDiagnosis() {
        ...
        const loadAll = async () => {
            ...
            try {
                const [s, m, c] = await Promise.all([
                    fetch(`${API}/api/symptoms`, { cache: "no-store" }),
                    fetch(`${API}/api/medications`, { cache: "no-store" }),
                    fetch(`${API}/api/chronics`, { cache: "no-store" }),
                ]);

                const [symData, medData, chrData] = await Promise.all([safeJson(s), safeJson(m), safeJson(c)]);

                const symNorm = Array.isArray(symData)
                    ? symData.map((s) => ({
                        id: s.id,
                        name: s.name || prettify(s.id),
                        description: s.description,
                    }))
                    : [];
                const medNorm = Array.isArray(medData) ? medData : [];
                const chrNorm = Array.isArray(chrData)
                    ? chrData.map((c) => ({
                        id: c.id,
                        name: c.name || prettify(c.id),
                    }))
                    : [];

                setSymptoms(symNorm);
                setMedications(medNorm);
                setChronicConditions(chrNorm);
    ...
    const selectedSymptomArray = useMemo(
        () => Object.entries(selectedSymptoms).map(([id, severity]) => ({ id, severity })),
        [selectedSymptoms]
    );
    ...
    const handleAnalyze = async () => {
        setError(null);
        if (!canAnalyze) {
            setError("Selecciona al menos un síntoma para analizar.");
            return;
        }
        setLoading(true);
        try {
            const body = {
                symptoms: selectedSymptomArray.map((s) => ({
                    id: isNaN(+s.id) ? s.id : +s.id,
                    severity: s.severity,
                })),
                allergies: selectedAllergies.map((id) => (isNaN(+id) ? id : +id)),
                chronics: selectedChronics.map((id) => (isNaN(+id) ? id : +id)),
            };
            ...
            const entry = {
                ...
                input: body,
                summary: sorted.map((r) => ({ disease: r.diseaseName, affinity: r.affinity, urgency: r.urgency })).slice(0, 5),
            };
    ...
    const affinitiesData = useMemo(
        () => (results || []).map((r) => ({ name: r.diseaseName, affinity: Math.round((r.affinity || 0) * 100) })),
        [results]
    );
    ...
    const downloadPDFClient = async () => {
        ...
        let y = 20;
        pdf.setFontSize(14);
        pdf.text("Informe de Análisis Clínico", 20, y);
        y += 18;
        pdf.setFontSize(10);
        pdf.text(`Fecha: ${new Date(generatedAt || Date.now()).toLocaleString()}`, 20, y);
        y += 10;
    ...
    const downloadPDFServer = async () => {
        const body = {
            symptoms: Object.entries(selectedSymptoms).map(([id, severity]) => ({
                id: isNaN(+id) ? id : +id,
                severity,
            })),
            allergies: selectedAllergies.map((id) => (isNaN(+id) ? id : +id)),
        ...
        try {
            const res = await fetch(`${API}/api/diagnosis/pdf`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify(body),
            });
            ...
            setError("No fue posible generar el PDF del servidor en este momento.");
        }
    };
            ...
            <div className="pi-container">
                <header className="pi-header">
                    ...
                    <p className="pi-muted">
                        Selecciona síntomas y severidad, registra alergias y condiciones crónicas. Luego solicita el análisis basado en reglas.
                    </p>
                </header>
                ...
                <section className="pi-grid">
                        ...
                        {symptoms.length === 0 ? (
                          ...
                          <span className="pi-muted" style={{ fontSize: 12 }}>
                            Severidad:
                                                    ...
                                                    <select
                                                        value={selectedSymptoms[s.id]}
                                                        onChange={(e) => changeSeverity(s.id, e.target.value)}
                                                        className="pi-select"
                                                    >
                                                        {SEVERITIES.map((sv) => (
                    ...
                    <div className="pi-card">
                        ...
                        <div style={{ marginBottom: 16 }}>
                            <div className="pi-muted" style={{ fontSize: 13, marginBottom: 6 }}>
                                ...
                                {medications.length === 0 ? (
                                    ...
                                    medications.map((m) => (
                                        <label key={m.id} className="pi-row" style={{ padding: "4px 0", cursor: "pointer" }}>
                                            <input
                                                type="checkbox"
                                                checked={selectedAllergies.includes(m.id)}
                                                onChange={() => toggleFromList(m.id, selectedAllergies, setSelectedAllergies)}
                                            />
                                ...
                                {chronicConditions.length === 0 ? (
                                    ...
                                    chronicConditions.map((c) => (
                                        <label key={c.id} className="pi-row" style={{ padding: "4px 0", cursor: "pointer" }}>
                                            <input
                                                type="checkbox"
                                                checked={selectedChronics.includes(c.id)}
                                                onChange={() => toggleFromList(c.id, selectedChronics, setSelectedChronics)}
                                            />
                ...
                <div className="pi-actions">
                    <button onClick={handleAnalyze} disabled={!canAnalyze} className="pi-btn primary">
                        {loading ? "Analizando…" : "Solicitar análisis"}
                    </button>
                    <button
                        onClick={() => {
                            setSelectedSymptoms({});
                    ...
                    {results.length > 0 && (
                        <>
                            <button onClick={downloadPDFClient} className="pi-btn success">
                                Descargar PDF (cliente)
                            </button>
                            <button onClick={downloadPDFServer} className="pi-btn success">
                ...
                <section id="diagnostic-report" className="pi-card">
                    <div style={{ display: "flex", gap: 16, alignItems: "center", marginBottom: 8 }}>
                        <h2 className="pi-title" style={{ margin: 0 }}>
                    ...
                    {results.length === 0 ? (
                        ...
                        <div style={{ display: "grid", gap: 24 }}>
                                        ...
                                        <tr key={i}>
                                            ...
                                            <td>
                                                {r?.conflicts?.length > 0 ? (
                                                    <span className="pi-badge" style={{ background: "#fef2f2", color: "#991b1b" }}>
                              {r.conflicts.join(", ")}
                            </span>
                                                ) : (
                            ...
                            <div>
                                ...
                                {rulesGlobal ? <pre className="pi-pre">{rulesGlobal}</pre> : <p className="pi-muted">El sistema no devolvió detalle de reglas activadas.</p>}
                            </div>
                        </div>
                    )}
                </section>
